...
class CrossProjectReference(BaseModel):
    ...
    # Reference metadata
    reference_type: str = Field(..., description="Type of reference (dependency, inspiration, etc.)")
    confidence_score: float = Field(..., ge=0.0, le=1.0, description="Confidence in reference relevance")
...
class FreshnessInfo(BaseModel):
    ...
    def calculate_category(cls, v: Optional[FreshnessCategory], values: Dict[str, Any]) -> FreshnessCategory:
    ...
    def generate_recommendations(cls, v: List[str], values: Dict[str, Any]) -> List[str]:
        ...
        if staleness < 60:  # < 1 minute
            return [
                f"✅ Fresh analysis of {scope_path}",
...
class StorageRequest(BaseModel):
    ...
    force_refresh: bool = Field(default=False, description="Force analysis even if content hash matches")
...
class HierarchicalTimestamp(BaseModel):
    ...
    change_source: Optional[str] = Field(None, description="File or component that triggered change")
...
class LifecycleRequest(BaseModel):
    ...
    analysis_ids: List[UUID] = Field(default_factory=list, description="Specific analysis IDs to target")
    
    # Criteria for bulk operations
    older_than_days: Optional[int] = Field(None, description="Target items older than N days")
    analysis_types: List[AnalysisType] = Field(default_factory=list, description="Target specific analysis types")
    staleness_threshold: Optional[float] = Field(None, description="Target items with staleness > threshold")
...
class CleanupPolicy(BaseModel):
    ...
    storage_threshold_mb: Optional[int] = Field(None, description="Cleanup when storage exceeds N MB")
