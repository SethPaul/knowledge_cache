...
class KnowledgeStore:
    ...
    async def get_component_architecture(
        ...
        main_analysis = await self.get_cached_analysis_with_freshness(
            component_scope, project_id
        )
        ...
        if include_dependencies:
            ...
            dependencies = await self._find_component_dependencies(
                component_scope, project_id
            )
            architecture["dependencies"] = [dep.dict() for dep in dependencies]
            
            # Find components that depend on this
            dependents = await self._find_component_dependents(
                component_scope, project_id
            )
        ...
        if include_relationships:
            ...
            related = await self.search.find_similar_components(
                reference_scope=component_scope,
    ...
    async def manage_knowledge_lifecycle(
        self,
    ...
    def _parse_db_row_to_analysis(self, row) -> AnalysisResult:
        ...
        if data['vector_embedding'] and isinstance(data['vector_embedding'], str):
    ...
    async def _find_existing_analysis(
        ...
        async with self.db_pool.acquire() as conn:
            row = await conn.fetchrow("""
                ...
                LIMIT 1
            """, project_id, target_scope, content_hash)
    ...
    def _extract_components(self, content: str) -> List[str]:
        ...
        import re
        components = re.findall(r'\b(?:class|function|component|module)\s+(\w+)', content, re.IGNORECASE)
    ...
    def _extract_structure(self, content: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "indentation_style": "spaces" if any(line.startswith('  ') for line in lines) else "tabs"
        }
    ...
    async def _store_analysis_in_db(self, analysis: AnalysisResult) -> None:
        ...
        async with self.db_pool.acquire() as conn:
            # Convert vector embedding to proper format for pgvector
            vector_data = None
    ...
    async def _find_lifecycle_targets(self, request: LifecycleRequest) -> List[AnalysisResult]:
        ...
        if request.older_than_days:
            query_parts.append(f" AND analysis_timestamp < NOW() - INTERVAL '{request.older_than_days} days'")
        ...
        query = " ".join(query_parts) + f" ORDER BY analysis_timestamp DESC LIMIT {request.batch_size}"
        
        async with self.db_pool.acquire() as conn:
            rows = await conn.fetch(query, *params)
            return [self._parse_db_row_to_analysis(row) for row in rows]
    
    async def _archive_analyses(self, analyses: List[AnalysisResult], request: LifecycleRequest) -> int:
