...
-- Try to enable pgvector extensions (graceful fallback if not available)
DO $$
BEGIN
    CREATE EXTENSION IF NOT EXISTS "pgvector";
    RAISE NOTICE 'pgvector extension enabled - vector search available';
...
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'vector extension not available - using fallback';
END
$$;
...
EXCEPTION WHEN OTHERS THEN
    RAISE NOTICE 'pg_trgm extension not available - some text search features disabled';
END
$$;
...
CREATE TABLE IF NOT EXISTS analysis_results (
    ...
    -- Vector embedding for semantic search (will be added conditionally below)
    -- vector_embedding vector(384), -- Added conditionally if pgvector available
    
    -- Indexing and constraints
    UNIQUE(project_id, target_scope, analysis_type, content_hash)
);

-- Add vector column conditionally if pgvector is available
DO $$
BEGIN
...
-- Vector similarity search index (conditional on pgvector availability)
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        ...
        IF NOT EXISTS (
            ...
            AND indexname = 'idx_analysis_results_vector'
        ) THEN
            -- Use ivfflat for broader compatibility (HNSW requires newer pgvector)
            CREATE INDEX idx_analysis_results_vector ON analysis_results USING ivfflat (vector_embedding vector_cosine_ops)
                WITH (lists = 100);
...
-- Function for semantic similarity search (conditional on vector column availability)
DO $$
BEGIN
    IF EXISTS (
        SELECT 1 FROM information_schema.columns 
        ...
        -- Create semantic search function if vector column exists
        ...
        RAISE NOTICE 'Created semantic_search function with vector support';
    ELSE
        -- Create fallback function without vector support
        CREATE OR REPLACE FUNCTION semantic_search(
        ...
        BEGIN
            RAISE NOTICE 'Vector search not available - falling back to text search';
            RETURN QUERY
            SELECT 
                ar.id,
                ar.target_scope,
            ...
            FROM analysis_results ar
            WHERE 
                (p_analysis_type IS NULL OR ar.analysis_type = p_analysis_type)
                AND (p_scope_filter IS NULL OR ar.target_scope LIKE p_scope_filter || '%')
            ORDER BY ar.analysis_timestamp DESC
            LIMIT p_limit;
        END;
        $func$ LANGUAGE plpgsql;
        RAISE NOTICE 'Created semantic_search function with text fallback';
    END IF;
END
$$;
...
-- Function to check schema capabilities
CREATE OR REPLACE FUNCTION get_schema_capabilities()
RETURNS JSONB AS $$
DECLARE
    capabilities JSONB;
BEGIN
    capabilities := jsonb_build_object(
        'vector_search', EXISTS (
            SELECT 1 FROM information_schema.columns 
            WHERE table_name = 'analysis_results' 
            AND column_name = 'vector_embedding'
        ),
        'pgvector_extension', EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'vector'),
        'text_search', true,
        'hierarchical_timestamps', true,
        'project_contexts', true,
        'cross_project_references', true,
        'archived_analyses', true,
...
COMMENT ON TABLE analysis_results IS 'Universal storage for all analysis types with optional vector embeddings';
...
COMMENT ON TABLE project_contexts IS 'Multi-project support with hierarchical relationships';
COMMENT ON TABLE cross_project_references IS 'Cross-project code relationships and dependencies';
COMMENT ON TABLE archived_analyses IS 'Long-term storage for archived analysis results';
COMMENT ON TABLE cleanup_policies IS 'Automated lifecycle management policies';
...
COMMENT ON FUNCTION get_schema_capabilities() IS 'Get current schema feature availability';
...
BEGIN
    ...
    RAISE NOTICE 'Vector Search: %', CASE WHEN capabilities->>'vector_search' = 'true' THEN 'ENABLED' ELSE 'DISABLED' END;
    RAISE NOTICE 'pgvector Extension: %', CASE WHEN capabilities->>'pgvector_extension' = 'true' THEN 'AVAILABLE' ELSE 'NOT AVAILABLE' END;
    RAISE NOTICE 'Text Search: ENABLED';
    RAISE NOTICE 'Project Support: ENABLED';
    RAISE NOTICE 'Cross-Project References: ENABLED';
    RAISE NOTICE 'Archived Analyses: ENABLED';
    RAISE NOTICE 'Cleanup Policies: ENABLED';
    RAISE NOTICE 'Lifecycle Operations: ENABLED';
    RAISE NOTICE '============================================';
END
$$; 